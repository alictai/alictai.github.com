$(document).ready(function(){
	// email address
	$(".mailto-email").attr("href","mailto:westchestercomputeradventures.com")

	// summer camp registration form online
	$(".reg-online").attr("href", "http://register.asapconnected.com/default.aspx?org=2346")

	// summer camp registration form pdf
	$(".reg-form").attr("href", "http://www.computeradventures.com/SuCampForm-2015-d.pdf");

	// party registration email
	$(".reg-party").attr("href","mailto:westchestercomputeradventures.com?subject=Computer%20Adventures%20Party%20Registration");

	// enrichment class registration pdf
	$(".reg-enrich").attr("href","http://www.computeradventures.com/ECForm-F2013-d.pdf");
});